...
import (
	...
	cryptocodec "github.com/artela-network/artela-rollkit/ethereum/crypto/codec"
...
func TestKeyring(t *testing.T) {
	...
	kr, err := keyring.New("artela", keyring.BackendTest, dir, mockIn, TestCodec, EthSecp256k1Option())
	...
	info, mnemonic, err := kr.NewMnemonic("foo", keyring.English, artelatypes.BIP44HDPath, keyring.DefaultBIP39Passphrase, EthSecp256k1)
	require.NoError(t, err)
	...
	hdPath := artelatypes.BIP44HDPath
...
func TestDerivation(t *testing.T) {
	bz, err := EthSecp256k1.Derive()(mnemonic, keyring.DefaultBIP39Passphrase, artelatypes.BIP44HDPath)
	require.NoError(t, err)
	require.NotEmpty(t, bz)
	...
	path := MustParseDerivationPath(artelatypes.BIP44HDPath)
	account, err := wallet.Derive(path, false)
	require.NoError(t, err)
	...
	// Equality of Artela implementation
	...
	// Equality of Eth and Artela implementation
	...
	// Inequality of wrong derivation path of Eth and Artela implementation
