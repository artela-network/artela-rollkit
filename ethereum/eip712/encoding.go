...
func decodeAminoSignDoc(signDocBytes []byte) (apitypes.TypedData, error) {
	...
	chainID, err := artelatypes.ParseChainID(aminoDoc.ChainID)
	if err != nil {
...
func decodeProtobufSignDoc(signDocBytes []byte) (apitypes.TypedData, error) {
	...
	chainID, err := artelatypes.ParseChainID(signDoc.ChainId)
	if err != nil {
...
func validatePayloadMessages(msgs []sdk.Msg) error {
	...
	for i, m := range msgs {
		...
		if !ok {
			return errors.New("unable to build EIP-712 payload: message is not a legacy message")
		}
