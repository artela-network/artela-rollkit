...
type Backend interface {
	...
	CosmosBlockByHash(hash common.Hash) (*coretypes.ResultBlock, error)
	CosmosBlockResultByNumber(height *int64) (*coretypes.ResultBlockResults, error)
...
func NewPublicFilterAPI(logger log.Logger, clientCtx client.Context, tmWSClient *rpcclient.WSClient, backend Backend) *PublicFilterAPI {
	api := &PublicFilterAPI{
		logger:    logger,
		clientCtx: clientCtx,
		backend:   backend,
