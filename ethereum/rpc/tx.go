...
import (
	...
	rpcutils "github.com/artela-network/artela-rollkit/ethereum/rpc/utils"
...
func (b *BackendImpl) GetTxMsg(ctx context.Context, txHash common.Hash) (*evmtypes.MsgEthereumTx, error) {
...
func (b *BackendImpl) PendingTransactions() ([]*sdktypes.Tx, error) {
	...
	if !ok {
		return nil, errors.New("failed to assert MempoolClient")
	}
	...
	result := make([]*sdktypes.Tx, 0, res.Count)
...
func (b *BackendImpl) GetResendArgs(args rpctypes.TransactionArgs, gasPrice *hexutil.Big, gasLimit *hexutil.Uint64) (rpctypes.TransactionArgs, error) {
	...
	if cfg == nil {
		...
		cfg = evmtypes.DefaultChainConfig().EthereumConfig(header.Number.Int64(), chainID)
	}
	...
	for _, tx := range pending {
		...
		// TODO, wantSigHash?
		msg, err := evmtypes.UnwrapEthereumMsg(tx, wantSigHash)
		if err != nil {
			// not ethereum tx
			continue
		}
...
func (b *BackendImpl) getTransaction(_ context.Context, txHash common.Hash) (*evmtypes.MsgEthereumTx, *rpctypes.RPCTransaction, error) {
...
func (b *BackendImpl) getTransactionByHashPending(txHash common.Hash) (*evmtypes.MsgEthereumTx, *rpctypes.RPCTransaction, error) {
	hexTx := txHash.Hex()
