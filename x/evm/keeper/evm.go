...
func (k Keeper) GetHashFn(ctx cosmos.Context) vm.GetHashFunc {
	return func(height uint64) common.Hash {
		...
		if err != nil {
			k.Logger().Error("failed to cast height to int64", "error", err)
			return common.Hash{}
		}
		...
		case ctx.BlockHeight() > h:
			...
			if err != nil {
				k.Logger().Debug("historical info not found", "height", h)
				return common.Hash{}
			}
...
func (k *Keeper) ApplyTransaction(ctx cosmos.Context, tx *ethereum.Transaction) (*types.MsgEthereumTxResponse, error) {
	var (
		bloom        *big.Int
	...
	msg, err := types.ToMessage(tx, signer, evmConfig.BaseFee)
	if err != nil {
