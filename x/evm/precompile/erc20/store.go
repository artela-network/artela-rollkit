...
func (c *ERC20Contract) GetProxyByDenom(ctx sdk.Context, denom string) ([]string, error) {
	store := prefix.NewStore(runtime.KVStoreAdapter(c.storeService.OpenKVStore(ctx)), evmtypes.KeyPrefixERC20Denom)
...
func (c *ERC20Contract) GetDenomByProxy(ctx sdk.Context, proxy common.Address) string {
	store := prefix.NewStore(runtime.KVStoreAdapter(c.storeService.OpenKVStore(ctx)), evmtypes.KeyPrefixERC20Address)
